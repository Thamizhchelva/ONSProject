...
conn = sqlite3.connect('database/ons.db',check_same_thread=False)
c = conn.cursor()
...
c.execute('''CREATE TABLE IF NOT EXISTS bookingtrends(
            timeLabel integer text,
            bookingData integer text
           )''')
...
if not adminuser:
    c.execute("INSERT INTO superusercreds VALUES ('ons','ons')")
    conn.commit()
...
@app.route('/save-trend', methods=['POST'])
def save_trend():
    time_label = request.json.get('timeLabel')
    booking_data = request.json.get('bookingData')

    c.execute('''INSERT INTO bookingtrends (timeLabel, bookingData) VALUES (?, ?)''', (time_label, booking_data))
    conn.commit()
    return 'OK', 200

@app.route('/get-trends')
def get_trends():
    c.execute('''SELECT * FROM bookingtrends''')
    trends = c.fetchall()
